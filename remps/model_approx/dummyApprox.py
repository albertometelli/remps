...
class MountainCarDummyApprox(ModelApprox):
    ...
    def __call__(self, states, actions, next_states, omega=None, initial_omega=10):
        ...
        with tf.variable_scope(self.name):
            ...
            # self.actions = tf.placeholder(self.default_dtype, (None, action_space), name="actions")
            # self.next_states = tf.placeholder(self.default_dtype, (None,output_space), name="next_states")
            ...
            pos_prime = vel + velDelta

            # velDelta = tf.clip_by_value(velDelta, -self.max_speed, self.max_speed)

            print("Vel delta shape:", velDelta.get_shape())

            self.pdf_pos = tf.distributions.Normal(pos_prime, scale=self.pos_std)

            self.pdf_vel = tf.distributions.Normal(velDelta, scale=self.vel_std)

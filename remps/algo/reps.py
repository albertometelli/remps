...
class REPMS():
	...
	theta = np.random.rand()
        ...
        self.param_v_ph = tf.placeholder(name='param_v', shape=(self.env.observation_space_size*2 +1, 1), dtype=self.dtype)
        ...
        weights = tf.exp(delta_v * self.param_eta_ph - tf.reduce_max(delta_v * self.param_eta_ph))
        weights_exp = delta_v * self.param_eta_ph - tf.reduce_max(delta_v * self.param_eta_ph)
        print("State kernel shape: ", state_kernel.get_shape())

        # For regularization add L2 reg term
        # use sum
        model_policy_loss = - tf.reduce_mean(tf.exp((delta_v * self.param_eta_ph - tf.reduce_max(delta_v * self.param_eta_ph)))
        ...
        model_policy_loss += self.L2_reg_projection * tf.add_n([tf.reduce_sum(tf.square(x - y)) for x,y in zip(self.policy.trainable_vars, other_policy.trainable_vars)])

        self.model_policy_tf_optimizer = self.tf_optimizer(model_policy_loss, var_list=self.model.trainable_vars + self.policy.trainable_vars) 
        
        # log of the policy dist
        logli = tf.log(tf.reduce_sum(tf.multiply(policy_tf, self.actions_one_hot_ph), axis=1, keepdims=True))
        print("Policy: ", logli.get_shape())

        # Policy loss (KL divergence, to be minimized)
        policy_loss = -tf.reduce_mean(logli * tf.exp(delta_v * self.param_eta_ph - tf.reduce_max(delta_v * self.param_eta_ph)))
        policy_regularizers = [tf.reduce_sum(tf.square(x)) for x in self.policy.trainable_vars]
        ...
        self.policy_tf_optimizer = self.tf_optimizer(policy_loss, var_list=self.policy.trainable_vars) 

        # Dual-related symbolics
        # Symbolic dual
        # debug purposes
        inside_log = tf.reduce_mean(tf.exp(delta_v * self.param_eta_ph - tf.reduce_max(delta_v) * self.param_eta_ph))
        inside_log_f = U.function(
            inputs=[self.rewards_ph, self.feat_diff_ph] + [self.param_eta_ph, self.param_v_ph],
            outputs=inside_log)

        #(1/self.param_eta_ph) * self.epsilon + 
        dual = (1/self.param_eta_ph)*self.epsilon + (1/self.param_eta_ph) * tf.log(
                    inside_log # + self.epsilon_small
                ) + tf.reduce_max(delta_v)
        # Add L2 regularization.
        dual += self.L2_reg_dual * \
                 (tf.square(self.param_eta_ph) + tf.square(1 / self.param_eta_ph)) #+ tf.reduce_sum(tf.square(self.param_v_ph)))
        ...
        exp_delta_v_eta_minus_max = tf.exp(delta_v * self.param_eta_ph - max_delta_v_eta)
        ...
        d_kl_pq = tf.reduce_mean((delta_v * self.param_eta_ph) * exp_delta_v_eta_minus_max)/mean_delta_v_eta_minus_max \
                  - tf.log(mean_delta_v_eta_minus_max) - max_delta_v_eta
        d_kl_pq_2 = tf.reduce_mean(delta_v * self.param_eta_ph * exp_delta_v_eta)/mean_delta_v_eta - tf.log(mean_delta_v_eta)
        d_kl_p_hat_q = tf.reduce_mean(tf.log(state_kernel + self.epsilon_small)-tf.log(other_state_kernel + self.epsilon_small))
        ...
        tf.summary.scalar("Theta", tf.reduce_sum(self.model.getTheta()))
        ...
        self.setPolicyParam = U.SetFromFlat(other_policy.trainable_vars, dtype=tf.float64)
    ...
    def _features(self, path):
        ...
        return np.hstack((o, o ** 2, pos*vel))# al, al**2, al**3))#, al, al ** 2, al ** 3, np.ones((l, 1))], axis=1)
    ...
    def train(self, samples_data, normalize_rewards=False):
        ...
        print("Dual after optimization: {}, Grad after optimization {}".format(dual_after, grad_after))
        ...
        theta_before = self.sess.run(self.model.getTheta())[0]

        logger.log('optimizing policy')
        inputs_dict = {self.rewards_ph:rewards, 
        ...
        print("-------------------- KL after projection ---------------- \n KL :{}".format(d_kl_p_hat_q_value))
        ...
        if self.projection_type == "joint":
            grad_loss = tf.gradients(model_policy_loss, self.policy.trainable_vars + self.model.trainable_vars)
            model_policy_loss_grad, model_policy_loss_value, state_kernel_val, variables, delta_v_val, weights, weights_exp = self.sess.run([grad_loss, model_policy_loss, state_kernel, self.policy.trainable_vars + self.model.trainable_vars, delta_v, self.weights, self.weights_exp], feed_dict=inputs_dict)
            variables_before = variables
            print(state_kernel_val[state_kernel_val == 0])
            print(actions[actions == 0])
            print(weights[weights!=0.])
            print(weights_exp)
            print("Before model policy optimization: \n Loss: {} \n Gradient: {} \n Variables: {} \n State kernel: {} \n Delta v: {} \n Weights :{}".format(model_policy_loss_value, model_policy_loss_grad, variables, state_kernel_val, delta_v_val, weights))

            self.model_policy_tf_optimizer.minimize(session=self.sess,
                                                    feed_dict=inputs_dict,
                                                    step_callback=policy_step_callback,
                                                    loss_callback=policy_loss_callback)
            model_policy_loss_grad, model_policy_loss_value, state_kernel_val, variables, delta_v_val = self.sess.run([grad_loss, model_policy_loss, state_kernel, self.policy.trainable_vars + self.model.trainable_vars, delta_v], feed_dict=inputs_dict)
            print(state_kernel_val[state_kernel_val == 0])
            print(actions[actions == 0])
            print("After model policy optimization: \n Loss: {} \n Gradient: {} \n Variables: {} \n State kernel: {}\n".format(model_policy_loss_value, model_policy_loss_grad, variables, state_kernel_val))
        else:
            ...
            policy_loss_grad,policy_loss_value, model_loss_grad, model_loss_value, variables, deltav_val  = self.sess.run([grad_policy, policy_loss, grad_model, model_loss, self.policy.trainable_vars + self.model.trainable_vars, delta_v], feed_dict=inputs_dict)
            variables_before = variables
            print("Before model policy optimization: \n Policy Loss: {} \n Model Loss: {} \n Policy Gradient: {} \n Model Gradient: {} \n Variables: {} \n Delta v {}".format(policy_loss_value, model_loss_value, policy_loss_grad, model_loss_grad, variables, deltav_val))

            # self.model_tf_optimizer.minimize(session=self.sess,
            #                                         feed_dict=inputs_dict,
            #                                         step_callback=policy_step_callback)
            self.policy_tf_optimizer.minimize(session=self.sess,
                                              feed_dict=inputs_dict,
                                              step_callback=policy_step_callback,
                                              loss_callback=policy_loss_callback)
        ...
        theta_after = self.sess.run(self.model.getTheta())[0]
    ...
    def pi(self, state, log=False):
        probs = self.sess.run(self.policy_tf, feed_dict={self.observations_ph:state})[0]
